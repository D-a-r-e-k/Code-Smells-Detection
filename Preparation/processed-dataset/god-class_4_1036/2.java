public static Element getDocument(String location) {
    try {
        ...
        PersistentValue persistentValue = PersistentValueManager.loadPersistentValue(Podcasting.class.getName() + "." + location);
            ...
            try {
                if (location.equals("local")) {
                    ...
                    if (PersistentValueManager.isAged(persistentValue)) {
                        PersistentValueManager.savePersistentValue(Podcasting.class.getName() + "." + location, content, 60 * 60 * 6);
                    }
                    stringReader.close();
                    stringReader = null;
                }
